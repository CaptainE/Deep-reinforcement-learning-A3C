...
class ModelBasedRL(object):
    ...
    def _train_policy(self, dataset):
        ...
        for _ in range(self._training_epochs):
            samples = dataset.random_iterator(self._training_batch_size)
            for states, actions, next_states, _, _ in samples:
    ...
    def run_q1(self):
        ...
        for r_num, (states, actions, _, _, _) in enumerate(self._random_dataset.rollout_iterator()):
            ...
            ### PROBLEM 1
            ### YOUR CODE HERE
            state=self._policy.predict(states[0],actions[0])
            pred_states.append(state)
            for a in range(1,len(actions)):
                state=self._policy.predict(state,actions[a])
                pred_states.append(state)
