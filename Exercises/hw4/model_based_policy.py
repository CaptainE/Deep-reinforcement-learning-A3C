...
class ModelBasedPolicy(object):
    ...
    def _dynamics_func(self, state, action, reuse):
        ...
        input_layer = tf.concat([state_ph,action_ph],axis=1)
        layer=utils.build_mlp(input_layer, self._state_dim,scope="name",
              n_layers=self._nn_layers,
              reuse=reuse)
        next_state_pred=utils.unnormalize(layer,self._init_dataset.delta_state_mean,self._init_dataset.delta_state_std)
        next_state_pred=next_state_pred+state
    ...
    def _setup_training(self, state_ph, next_state_ph, next_state_pred):
        ...
        #b
        actual_state_diff=utils.normalize(actual_state_diff,self._init_dataset.delta_state_mean,self._init_dataset.delta_state_std)
        pred_state_diff=utils.normalize(pred_state_diff,self._init_dataset.delta_state_mean,self._init_dataset.delta_state_std)
        #c
        loss = tf.reduce_mean(0.5 * tf.square((actual_state_diff-pred_state_diff)))
        #d 
    ...
    def _setup_action_selection(self, state_ph):
        ...
        ### PROBLEM 2
        ### YOUR CODE HERE
        losses = np.zeros(self._num_random_action_selection)
        state_ph = tf.ones([self._num_random_action_selection, 1]) * state_ph[0]
        for i in range(self._horizon):
            random_sampled_action = tf.random_uniform(shape=[self._num_random_action_selection, self._horizon, self._action_dim], minval=self._action_space_low, maxval=self._action_space_high, dtype=tf.float32)
            if i == 0:
                first_action = random_sampled_action[:,i,:]
            next_state_pred = self._dynamics_func(state_ph,random_sampled_action[:,i,:],reuse=True)
            losses += self._cost_fn(state_ph,random_sampled_action[:,i,:],next_state_pred)
            state_ph = next_state_pred
        best_action = first_action[tf.argmin(losses,axis=0)]
    ...
    def train_step(self, states, actions, next_states):
        ...
        ### PROBLEM 1
        ### YOUR CODE HERE
        _,c = self._sess.run([self._optimizer, self._loss], 
                                feed_dict={
