...
class RBF(Density_Model):
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            deltas = []
            for state in states:
                deltas.append([state - mean for mean in self.means])
...
class Exemplar(Density_Model):
    ...
    def update(self, state1, state2, target):
        ...
        opt, elbo, ll, kl = self.sess.run([self.update_op, self.elbo, self.log_likelihood, self.kl], feed_dict={self.state1: state1, self.state2: state2, self.discrim_target: target})
    ...
    def get_likelihood(self, state1, state2):
        ...
        likelihood = self.sess.run([self.likelihood], feed_dict={self.state1: state1, self.state2: state2, self.discrim_target: np.ones([state2.shape[0],1])})
