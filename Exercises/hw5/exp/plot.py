...
def plot_data(data, value="AverageReturn"):
    ...
    for i, iteration in enumerate(data.Iteration):
        if iteration not in d or (iteration in d and d[iteration] < data.AverageReturn[i]):
            d[iteration] = data.AverageReturn[i]

    sns.set(style="darkgrid", font_scale=1.5)
    plt.plot(range(100), list(d.values()))
    plt.xlabel('Iteration')
    plt.ylabel('Mean reward')
    plt.title('PM_rbf_bc0.01_s8_sig0.2')
